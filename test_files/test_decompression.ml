...
let blaise_pascal_ () =
  ...
  let check = 
    let command = (* obligé de faire un début de commande avec 'bash' car la commande de base ne se lance pas en bash, donc impossible d'avoir les variables *)
      "bash -c 'cmp -n \"$(stat -c%s test_files/ressources/Blaise_Pascal/Blaise_Pascal.txt)\" \
                test_files/ressources/Blaise_Pascal/Blaise_Pascal.txt \
                <(head -c \"$(stat -c%s test_files/ressources/Blaise_Pascal/Blaise_Pascal.txt)\" " ^
      output ^
      ")'"
